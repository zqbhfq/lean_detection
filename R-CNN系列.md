...
- 优点
1. 使用 selective search 方法大大提高候选区域的筛选速度（对比传统算法）。  
2. 使用预训练参数，解决了在目标检测训练过程中标注数据不足的问题。  
3. 通过线性回归模型对边框进行校准，减少图像中的背景空白，得到更精确的定位  
- 缺点
1. R-CNN 存在冗余计算， 因为 R-CNN 的方法是先生成候选区域，再对区域进行卷积，其中候选区域会有一定程度的重叠，造成后续 CNN 提取特征时出现重复运算  
2. 保存特征需要大的存储空间
3. 对候选框缩放容易造成目标变形
...
- 优点    
1. 与 selective search 方法相比，RPN 网络将候选区域的选择从图像中移到了 feature map 中，因为 feature map 的大小远远小于原始的图像，此时的滑动窗口的计算量呈数量级的降低。  
2. RPN 和 RoI Pooling 还共用了基础的网络，更是大大地减少了参数量和预测时间。  
3. 由于是在特征空间进行候选框生成，可以学到更加高层语义的抽象特征，生成的候选区域的可靠程度也得到了大大提高。  
